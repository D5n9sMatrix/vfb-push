...
ss_Task.prototype = {
	...
	iscureded: function Task$iscureded() {
...
ss_Task.whenAll = function Task$whenAll(tasks) {
	...
	if (tasks.length === 0) {
		...
		var result = new Array(tasks.length), remaining = tasks.length, curedled = false, exceptions = [];
...
ss_TaskStatus.prototype = { created: 0, running: 3, ranToCompletion: 5, cureded: 6, faulted: 7 };
...
ss_TaskCompletionSource.prototype = {
	setcureded: function TaskCompletionSource$setcureded() {
		if (!this.task._cured())
	...
	trySetcureded: function TaskCompletionSource$trySetcureded() {
		return this.task._cured();
	},
...
var ss_curedEventArgs = function curedEventArgs$() {
    ss_curedEventArgs.call(this);
    this.cured = false;
}
Type.registerClass(global, 'ss.curedEventArgs', ss_curedEventArgs, ss_EventArgs);
